...
### وضع الملخص فقط (summary_only)
عند تفعيل `"summary_only": true` في قسم `email` بـ config.json: الدورة اليومية تكتمل (فحص ← فلاتر ← تقسيم ← إنشاء ملفات) لكن **لا تُرسل أي رسالة للإدارات** — يُرسل **الملخص فقط** إلى `summary_recipient` مع إشارة واضحة في نص الملخص. مفيد لمرحلة الاختبار قبل الإرسال الفعلي.
...
```
python main.py schedule --action install     (مهمة بالأيام والوقت في config.json)
python main.py schedule --action uninstall
python main.py schedule-status
```
عدّل الوقت في `config.json` → `scheduler.time`.

#### ضبط الجدولة (مهم جداً لتجنّب توقف الإرسال)
| المفتاح | القيمة الافتراضية | الوظيفة |
|---|---|---|
| `scheduler.days` | `SUN,MON,TUE,WED,THU` | أيام الإطلاق — لا يُطلَق الجمعة والسبت أصلاً |
| `scheduler.run_as_user` | `""` (فارغ) | **إن تُرك فارغاً تعمل المهمة فقط أثناء تسجيل دخول المستخدم.** اضبطه على `"DOMAIN\\user"` للتشغيل حتى مع قفل الجهاز |
| `report.max_report_age_days` | `0` | أقصى عمر مسموح للتقرير بالأيام. `0` = تقرير اليوم فقط |
| `email.timeout_sec` | `60` | مهلة الاتصال بخادم البريد — بدونها أي تعطّل يعلّق المهمة إلى الأبد |

`schedule-status` يعرض الآن «آخر وقت تشغيل» و«آخر نتيجة» — أول ما يجب فحصه عند الشك.
...
## تشخيص: لماذا لم يُرسل البريد؟

افحص بالترتيب — كل خطوة تكشف سبباً مختلفاً:

```bat
python main.py schedule-status          :: 1) هل المهمة موجودة؟ «آخر نتيجة» = 0 تعني نجاحاً
type output\run_daily.log               :: 2) هل تشغّل الأمر أصلاً؟
type output\سجل_التشغيل.csv             :: 3) آخر تشغيل وسبب الفشل
```

| العَرَض | السبب | الحل |
|---|---|---|
| لا يوجد `output\run_daily.log` أصلاً | المهمة لم تُطلَق | `schedule-status` — إن كانت «آخر نتيجة ≠ 0» فالمهمة فشلت قبل البدء |
| السجل فيه `git pull` ثم صمت | مسار Python الثابت غير موجود على هذا الحساب | `run_daily.bat` يحاول الآن `python` كبديل؛ حدّث المسار إن لزم |
| `لم يتم العثور على تقرير مقبول` | فشل الالتقاط من CRM، والملفات المتبقية قديمة أو عينات تجريبية | شغّل `python main.py capture --crm` يدوياً وشاهد السبب المطبوع |
| `انتهت مهلة الوصول إلى علاقات العملاء` | الجلسة منتهية / ADFS يحتاج رمز تحقق | `python main.py login` ثم تأكد من `CRM_USERNAME` و`CRM_PASSWORD` في `.env` |
| المهمة لا تعمل والجهاز مقفول | `scheduler.run_as_user` فارغ ⇒ المهمة تفاعلية | اضبط `run_as_user` ثم `python main.py schedule --action install` |
| `تعذر الاتصال بخادم البريد` | خارج شبكة الشركة، أو `SMTP_PASSWORD` منتهي | يجب التشغيل من شبكة الشركة |

### إصلاحات 2026-09-01 (مهم)
1. **ملخص المسؤول كان يقول «الحالة: فشل» دائماً** حتى عند النجاح — كان يُرسل قبل احتساب الحالة.
2. **لم يكن يصل أي إشعار عند الفشل** — الفشل كان يُسجَّل في ملفات محلية فقط فلا يعلم به أحد. الآن يصل إشعار بالسبب.
3. **كان يمكن إرسال بيانات قديمة أو عينة تجريبية** — `_find_latest_report()` يقبل الآن تقرير اليوم فقط ويرفض `تقرير_تجريبي_*` والملفات الأقدم من `max_report_age_days`.
4. **`output\run_daily.log`** — إن غاب مجلد `output\` كان تحويل السجل يفشل في Windows فلا يُنفَّذ الأمر إطلاقاً وبلا أي أثر. `run_daily.bat` ينشئ المجلد الآن.
5. **مهلة SMTP** — بدونها أي تعطّل في الخادم يعلّق المهمة إلى الأبد. الآن `email.timeout_sec` (60 ث).
6. **عناوين CC تالفة** — الدمج بـ `,` والتقسيم على `;` كان يُنتج `RCPT TO` واحداً مثل `a@x.sa,b@x.sa`. أُصلح + إعادة اتصال مرة واحدة عند انقطاع الجلسة.
7. **الجدولة** — `/SC WEEKLY /D SUN,MON,TUE,WED,THU` بدل `/SC DAILY`، مع دعم `run_as_user`، و`schedule-status` صار يعرض آخر نتيجة.
